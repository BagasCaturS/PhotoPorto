...
export default function EditJournalPage() {
  ...
  useEffect(() => {
    if (!isLoading && !isAdmin) {
      router.push("/auth/login?redirect=/journal")
    }
  ...
  const handleSubmit = async (e: React.FormEvent, asDraft = false) => {
    ...
    setError(null)
    try {
      const coverUrl = coverFile ? await uploadCover() : coverSrc
                ...
                {!title && !currentCover && !content ? (
                  ...
                  <div className="overflow-hidden rounded-2xl border border-border dark:border-dark-border">
                    <div className="relative h-[40vh] min-h-[260px] overflow-hidden bg-foreground dark:bg-dark-foreground">
                      {currentCover ? (
                        <img src={currentCover} alt="" className="h-full w-full object-cover opacity-60" />
                      ) : (
